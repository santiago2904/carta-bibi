...
Gemelo del repo `carta/` (para Kata): misma arquitectura, distinta paleta
(rosa/dorado), distintos íconos de fondo y distinto topic de notificaciones.
...
assets/background.js  íconos flotantes (llama, flor, olas, casco, helado, lego, 🍗)
...
1. Copia `cartas/carta-1.js` a `cartas/carta-2.js` y edita:

```js
(window.LETTERS = window.LETTERS || []).push({
  n: 2,                                  // número, sale en el menú y en los push
  file: "carta-2.html",                  // a dónde apunta el menú
  ...
  coverTitle: "Para ti, Bibi",           // portada (admite HTML)
...
2. Copia `carta-1.html` a `carta-2.html` y cambia **solo** el `<script>` del
   contenido y el `<title>`:

```html
<title>Carta #2</title>
...
<script src="cartas/carta-2.js"></script>
```
...
Topic: **`carta-bibi-santi-9k2mx7`** (en `assets/notify.js:4`).
...
La versión vieja de este repo usaba `sendBeacon` para el evento de Spotify (para
que sobreviviera a la navegación). Ya no hace falta: el `fetch` con
`keepalive: true` de `notify.js` cumple lo mismo.
...
Para ver qué llegó al topic:

```
https://ntfy.sh/carta-bibi-santi-9k2mx7/json?poll=1&since=1h
```
...
- El contenido de las cartas es personal: **nunca reescribir el texto de una
  carta existente** salvo que se pida explícitamente. La carta #1 se migró
  palabra por palabra desde el `index.html` original.
